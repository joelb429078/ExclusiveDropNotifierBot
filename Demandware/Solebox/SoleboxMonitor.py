...
def monitor():
    ...
    #First scrape will not notify all products
    start = 1

    #Gettings and initislaing proxies and headers
    proxy_no = 0
    ...
    while True:
        try:
            #Request to site and stores products 
            ...
            for item in items:
                ...
                if keywords == "":
                    ...
                    #For each keyword will check if any changes have happeneed
                    for key in keywords:
            ...
            #Roatates all the headers
            headers = {'User-Agent': user_agent_rotator.get_random_user_agent()}
            
            if CONFIG['PROXY'] != "":
                #If proxy set, rotates if there are multipl proxies
